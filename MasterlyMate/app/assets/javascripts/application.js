...
function onLanguageChanged(switcher)
{
	...
	var selectedValue = switcher.find('option:selected').val();
...
{
	if($(".prog_init").length > 0)
	{
		$(".prog_init").first().trigger("click");
	}
}
...
function initExamRadiobuttons()
{
	//$("input[name='ExamSucceeded'][value='0']").attr("checked","checked");
	$("input[name='ExamSucceeded']").change(function() {
		updateCurrentExam();
	});
}

function updateCurrentExam()
{
	selectedValue = $("input[name='ExamSucceeded']:checked").val();
	paramString = "assessment="+$(".update_info").attr("assessment")+"&exam="+
	$(".update_info").attr("exam")+"&wbt="+$(".update_info").attr("wbt")+"&increase="+(selectedValue == "1");
	$.ajax({
		url: "/users/"+$(".update_info").attr("user")+"/updateExam?"+paramString
	});
}

function updateProgressbar(id, progressValue, title)
{
	$("#"+id).progressbar({ value: progressValue }).attr("title", title);
}
