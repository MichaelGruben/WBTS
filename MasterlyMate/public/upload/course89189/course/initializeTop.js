...
{
    var themedir = gblThemeDir;
  if(navigator.appName == "Microsoft Internet Explorer" || (forceRelativePath == true))
  {
...
function openResources()
{
  ...
  {
    MM_openBrWindow('Resources.html','Resources','scrollbars=yes,resizable=no,width=720,height=500,left=60,top=100')
  }
}
...
function setLogout()
{
  ...
  {
      if (isSCORM_1_2){LMSSetValue("adl.nav.request", "exit");
        LMSSetValue("cmi.core.exit","exit");
        //LMSSetValue("adl.nav.request", "suspendAll");
      }
      else{LMSSetValue("adl.nav.request", "exit");
        LMSSetValue("cmi.exit","exit"); //scorm 2004
        //LMSSetValue("adl.nav.request", "suspendAll");
...
function resetCourse(ignoreCorrect)
{
    ...
    if (true)
    {
        ...
        { 
            for (var i = 0; i<=arrInteractions.length -1; i++)
            {//null out the current session interactions
        ...
        { 
            for (var i = 0; i<=arrPreviousSessionInteractions.length -1; i++)
            {//null out previous session interactions
            //if (arrPreviousSessionInteractions[i].id == interactionID)
                //return arrPreviousSessionInteractions[i];
